...
options = {
  ...
  -- set frequency of imu to be the same as gps and range scan
  imu_sampling_ratio = 1e-1,
}
...
POSE_GRAPH.optimize_every_n_nodes = 3.2e2
...
POSE_GRAPH.global_sampling_ratio = 3e-3

-- not trust imu rotation
POSE_GRAPH.optimization_problem.rotation_weight = 1e-2

-- close loop with large Z for z-drift overtime
...
POSE_GRAPH.constraint_builder.fast_correlative_scan_matcher_3d.linear_xy_search_window = 2e1
